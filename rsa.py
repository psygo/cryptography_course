...
# Step 3: Compute lambda(n) = lcm(a, b) = |ab| / gcd(a, b) = lcm(lambda(p), lambda(q))
...
# Step 5: Determine d: solve for d the equation d * e = 1 (mod lambda(n))
d = get_d(e, lambda_n)
...
message = "Alice is awesome" # too big to be inside a single message
for m_c in message:
    c = ord(m_c)**e % n
    print(c, " ", end='')
print()
# Now you can use frequency analysis to break it.
# You need some randomness to make it secure.
# Adding padding helps counteract the deterministic part of RSA (Chosen Plaintext Attack).
